...
namespace services
{
    ...
    FlasOnStBootloaderCommunicator<T, Args...>::FlasOnStBootloaderCommunicator(Args... args, StBootloaderCommunicator& communicator)
        : T(std::forward<Args>(args)...)
        , flashOnStBootloaderCommunicator(*this, communicator)
    {}

    template<class T, class... Args>
    void FlasOnStBootloaderCommunicator<T, Args...>::WriteBuffer(infra::ConstByteRange buffer, uint32_t address, infra::Function<void()> onDone)
    {
        flashOnStBootloaderCommunicator.WriteBuffer(buffer, address + 0x8000000, onDone);
    }

    template<class T, class... Args>
    void FlasOnStBootloaderCommunicator<T, Args...>::ReadBuffer(infra::ByteRange buffer, uint32_t address, infra::Function<void()> onDone)
    {
        flashOnStBootloaderCommunicator.ReadBuffer(buffer, address + 0x8000000, onDone);
    }

    template<class T, class... Args>
    void FlasOnStBootloaderCommunicator<T, Args...>::EraseSectors(uint32_t beginIndex, uint32_t endIndex, infra::Function<void()> onDone)
    {
        flashOnStBootloaderCommunicator.EraseSectors(beginIndex, endIndex, onDone);
    }

    template class FlasOnStBootloaderCommunicator<hal::FlashHomogeneous, uint32_t, uint32_t>;
    template class FlasOnStBootloaderCommunicator<hal::FlashHeterogeneous, infra::MemoryRange<uint32_t>>;
}
