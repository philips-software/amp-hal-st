...
namespace hal
{
    namespace
    {
        ...
        constexpr uint8_t NumberOfBleMemoryBlocks(uint16_t maxAttMtuSize)
        {
            return static_cast<uint8_t>(BLE_MBLOCKS_CALC(PrepareWriteListSize(maxAttMtuSize), maxAttMtuSize, maxNumberOfBleLinks));
        }
        ...
        void ShciInitialization(GapSt::BleBondStorage bleBondStorage, const GapSt::Configuration& configuration)
        {
            ...
            SHCI_C2_Ble_Init_Cmd_Packet_t bleInitCmdPacket = {
                { { 0, 0, 0 } }, // Header (unused)
                {
                    ...
                    numAttrRecord,  // Maximum number of GATT Attributes
                    numAttrServ,  // Maximum number of Services that can be stored in the GATT database
                    attrValueArrSize, // Size of the storage area for Attribute values
                    maxNumberOfBleLinks,
                    ...
                    bleStackOptions,
                    0,   // HW version (unused)
                    maxNumberOfConnectionOrientedChannels,  // Maximum number of connection-oriented channels in initiator mode
            ...
            really_assert(SHCI_C2_BLE_Init(&bleInitCmdPacket) == SHCI_Success);
        }
...
#if defined(STM32WB)
        ShciInitialization(bleBondStorage, configuration);
#elif defined(STM32WBA)
        BleStackInitialization(bleBondStorage, configuration);
#endif
