#include "hal_st/stm32fxxx/UartStmDuplexDmaG4.hpp"
...
namespace hal
{
    ...
    UartStmDuplexDmaG4::UartStmDuplexDmaG4(infra::MemoryRange<uint8_t> rxBuffer, hal::DmaStmG4& dma, uint8_t aUartIndex, GpioPinStm& uartTx, GpioPinStm& uartRx, const Config& config)
        : rxBuffer(rxBuffer)
        ...
        , transmitDmaChannel(dma, config.dmaChannelTx, &peripheralUart[uartIndex]->TDR, [this]()
              { TransferComplete(); })
        , receiveDmaChannel(
              dma, config.dmaChannelRx, &peripheralUart[uartIndex]->RDR, [this]()
    ...
    UartStmDuplexDmaG4::UartStmDuplexDmaG4(infra::MemoryRange<uint8_t> rxBuffer, hal::DmaStmG4& dma, uint8_t aUartIndex, GpioPinStm& uartTx, GpioPinStm& uartRx, GpioPinStm& uartRts, GpioPinStm& uartCts, const Config& config)
        : rxBuffer(rxBuffer)
        , uartIndex(aUartIndex - 1)
        , uartTx(uartTx, PinConfigTypeStm::uartTx, aUartIndex)
        , uartRx(uartRx, PinConfigTypeStm::uartRx, aUartIndex)
    ...
    UartStmDuplexDmaG4::~UartStmDuplexDmaG4()
    {
    ...
    void UartStmDuplexDmaG4::SendData(infra::MemoryRange<const uint8_t> data, infra::Function<void()> actionOnCompletion)
    {
        if (!data.empty())
        {
    ...
    void UartStmDuplexDmaG4::ReceiveData(infra::Function<void(infra::ConstByteRange data)> dataReceived)
    {
    ...
    void UartStmDuplexDmaG4::Configure(const Config& config)
    {
    ...
    void UartStmDuplexDmaG4::RegisterInterrupt(const Config& config)
    {
        if (config.priority)
    ...
    void UartStmDuplexDmaG4::TransferComplete()
    {
    ...
    void UartStmDuplexDmaG4::Invoke()
    {
